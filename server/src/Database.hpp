...
public:
    ...
    bool insert_fix(fix::Fix fix);

    // Return the fix if found, std::nullopt otherwise.
    std::optional<fix::Fix> get_fix(uint32_t date);

    // Return first fix if it exists, std::nullopt otherwise.
    std::optional<fix::Fix> get_first_fix();
    ...
    std::vector<fix::Fix> get_fix_range(uint32_t start, uint32_t end);
